...
@Component({
  ...
  imports: [RoomCard],
  template: `
    <section class="w-full py-12 bg-base-een-100">
      <div class="w-full max-w-7xl mx-auto px-4 flex flex-col items-center">
        <h2 class="text-3xl md:text-5xl font-light text-primary-900 tracking-wide mb-8" id="topList">
          Uitgelichte <span class="font-bold text-accent">Koten</span>
        </h2>
        
        @if (isLoading()) {
        <div class="text-gray-500">Koten laden...</div>
        } @else if (!rooms() || rooms()?.length === 0) {
        <div class="text-gray-500">Geen highlighted kamers gevonden.</div>
        } @else {
        <div
        ...
        <!-- Pagination controls -->
        <div class="flex flex-wrap justify-center items-center gap-2 mt-12 select-none">
          <button
            (click)="prevPage()"
            [disabled]="currentPage() === 1"
            class="h-10 w-10 flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all text-gray-600 bg-white shadow-sm"
            title="Vorige pagina"
          >
            <svg
          ...
          <button
            (click)="goToPage(page)"
            class="h-10 min-w-10 px-3 flex items-center justify-center rounded-lg text-sm font-bold transition-all border shadow-sm"
            [class.bg-accent-500]="currentPage() === page"
            [class.text-white]="currentPage() === page"
            [class.border-accent-500]="currentPage() === page"
            [class.bg-white]="currentPage() !== page"
            [class.text-base-twee-700]="currentPage() !== page"
