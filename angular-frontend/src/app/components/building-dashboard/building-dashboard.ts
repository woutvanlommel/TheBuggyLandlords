  ...
  template: `
    ...
    <div class="space-y-4" [class]="showAddBuildingModal ? 'min-h-180' : 'min-h-150px'">
      ...
      @if (!loading && buildings.length === 0) {
        <div
          class="text-center py-8 text-base-twee-500 bg-base-een-50/50 rounded-xl border border-base-twee-100"
        >
          Geen items gevonden.
        </div>
      }
      ...
      @for (building of buildings; track building.favorite_room_id || building.id) {
        @if (isLandlord) {
          <div
            class="bg-base-een-100/30 border border-base-twee-200/50 rounded-xl overflow-hidden mb-4 transition-all hover:shadow-sm"
          >
            <div
            ...
            >
              <div class="flex items-center gap-4">
                <div
                  class="w-10 h-10 rounded-full bg-white border border-base-twee-100 flex items-center justify-center text-primary-600 shadow-sm"
                >
                  <svg
                ...
                <div>
                  <h4 class="font-bold text-base-twee-900">
                    {{ building.street?.street || building.street }}
                    {{ building.housenumber || building.number }}
                  </h4>
                  <p class="text-xs text-base-twee-500">
                    {{ building.city || building.place?.place }} •
                    {{ building.rooms?.length || 0 }} Kamers •
                    <span
              ...
              <div class="border-t border-base-twee-200/50 bg-white/50 backdrop-blur-sm">
                <div class="overflow-x-auto">
                  <table class="w-full text-left text-sm text-base-twee-700">
                    <thead
                      class="text-xs uppercase text-base-twee-500 font-semibold border-b border-base-twee-200/50 bg-base-een-50/30"
                    >
                      <tr>
                    ...
                    <tbody class="divide-y divide-base-twee-200/50">
                      @for (room of building.rooms; track room.id) {
                        <tr class="hover:bg-base-een-100/50 transition-colors">
                          <td class="px-4 py-3 font-medium text-base-twee-900">
                            {{
                              room.name ||
                                (room.roomtype?.type || 'Kamer') + ' ' + (room.room_number || '')
                            }}
                          </td>
                          <td class="px-4 py-3">
                            @if (room.active_contract?.user; as tenant) {
                              <div class="flex items-center gap-2">
                                <div
                                  class="w-6 h-6 rounded-full bg-secondary-100 text-secondary-700 flex items-center justify-center text-xs font-bold"
                                >
                                  {{ tenant.fname.charAt(0) }}
                                </div>
                                <span class="truncate max-w-[120px]"
                                  >{{ tenant.fname }} {{ tenant.name }}</span
                                >
                              </div>
                            } @else {
                          ...
                          <td class="px-4 py-3 text-right">
                            <a
                              ...
                              class="text-primary-600 hover:text-primary-800 hover:underline text-xs font-medium cursor-pointer"
                              >Beheer</a
                            >
                          </td>
                        </tr>
                      ...
                      @if (!building.rooms || building.rooms.length === 0) {
                        <tr>
                          <td
                            colspan="4"
                            class="px-4 py-6 text-center text-base-twee-400 text-sm italic"
                          >
          ...
          <div class="relative mb-4 group">
            ...
            >
              <div
                class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-primary-50 rounded-full blur-2xl opacity-50 group-hover:opacity-100 transition-opacity"
              ></div>

              <div class="flex items-center gap-5 relative z-10 pr-10">
                <div
                  class="w-12 h-12 rounded-2xl bg-base-een-50 flex items-center justify-center text-base-twee-400 group-hover:bg-primary-600 group-hover:text-white transition-all duration-300 shadow-sm border border-base-twee-100 group-hover:border-primary-600"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                ...
                <div class="flex-1">
                  <h4
                    class="font-bold text-lg text-base-twee-900 group-hover:text-primary-700 transition-colors"
                  >
      ...
      @if (showAddBuildingModal) {
        <div
          class="absolute w-full h-full top-0 left-0 inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-base-twee-900/20"
          (click)="closeModal()"
        >
          <div
          ...
          >
            <div
              class="px-8 py-5 border-b border-base-twee-100 flex items-center justify-between bg-gray-50"
            >
              <h3 class="text-xl font-bold text-base-twee-900">Nieuw Gebouw Toevoegen</h3>
              <button
                (click)="closeModal()"
                class="p-2 hover:bg-gray-200 rounded-full transition-colors text-base-twee-500"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6"
            ...
            <div class="p-8 overflow-y-auto">
              <div class="space-y-6">
                <div>
                  <label
                    class="block text-xs font-bold text-base-twee-500 uppercase tracking-widest mb-1.5 ml-1"
                    >Straatnaam</label
                  >
                  <div class="relative">
                    <input
                      ...
                      (input)="onStreetInput()"
                      class="w-full px-4 py-3 rounded-xl border border-base-twee-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 outline-none transition-all font-medium"
                      placeholder="bv. Naamsestraat"
                    />
                    @if (suggestLoading) {
                      <div
                        class="absolute right-3 top-3.5 w-5 h-5 border-2 border-primary-500 border-t-transparent rounded-full animate-spin"
                      ></div>
                    }

                    @if (suggestions.length > 0) {
                      <div
                        class="absolute z-10 left-0 right-0 mt-1 bg-white rounded-xl shadow-xl border border-base-twee-100 overflow-hidden max-h-48 overflow-y-auto"
                      >
                        @for (suggestion of suggestions; track $index) {
                          <div
                            (click)="selectSuggestion(suggestion)"
                            class="px-4 py-3 hover:bg-primary-50 cursor-pointer text-sm border-b border-gray-50 last:border-0"
                          >
                            <div class="font-bold text-base-twee-800">{{ suggestion.street }}</div>
                            <div class="text-xs text-base-twee-500">
            ...
            <div class="px-8 py-5 bg-gray-50 border-t border-base-twee-100 flex gap-3">
              <button
                (click)="closeModal()"
                class="flex-1 py-3 rounded-xl font-bold text-base-twee-600 hover:bg-base-twee-200 transition-colors"
              >
                Annuleren
              </button>
              <button
                ...
                class="flex-2 py-3 rounded-xl font-bold text-white bg-primary-600 hover:bg-primary-700 shadow-lg shadow-primary-600/20 disabled:opacity-50 transition-all flex items-center justify-center gap-2"
              >
                @if (loading) {
                  <div
...
export class BuildingDashboard implements OnInit {
  ...
  ngOnInit() {
    ...
    if (user) {
      ...
      this.isLandlord =
        Number(user.role_id) === 2 || roleName === 'landlord' || roleName === 'verhuurder';
    }
    this.loadBuildings();
  }
