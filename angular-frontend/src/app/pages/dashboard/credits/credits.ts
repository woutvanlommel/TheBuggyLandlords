...
@Component({
  ...
  template: `
    <div class="space-y-8">
      <!-- HEADER: BALANCE -->
      <div
        class="p-8 text-center bg-base-een-100/50 backdrop-blur-sm rounded-xl border border-primary-100/50 shadow-sm relative overflow-hidden group"
      >
        <div
          class="absolute inset-0 bg-linear-to-r from-primary-100/20 to-secondary-100/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
        ></div>
        ...
        <div class="mt-4 flex flex-col items-center justify-center relative z-10">
          <span
            class="text-5xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-primary-600 to-secondary-600"
          >
            {{ balance }}
          </span>
          <span class="mt-1 text-sm font-bold text-base-twee-500 uppercase tracking-widest"
            >Beschikbaar</span
          >
        </div>
      ...
      <section>
        <div class="flex items-center justify-between mb-4 px-2">
          ...
          <span class="text-xs text-secondary-600 font-semibold cursor-pointer hover:underline"
            >View History</span
          >
        </div>
      ...
      <section
        *ngIf="isLandlord"
        class="bg-base-een-100/50 rounded-2xl border border-dashed border-base-twee-300 p-6"
      >
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p class="text-xs text-base-twee-500">
              Boost visibility for 1 credit/day per property.
            </p>
          </div>
        </div>
      ...
      <section class="mt-8 text-center bg-primary-100 rounded-xl p-6">
        ...
        <p class="text-primary-600 text-sm mb-4">
          Our support team is available 24/7 for payment issues.
        </p>
        <a
          href="mailto:KotCompass@outlook.com"
          class="inline-block bg-primary text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-primary-700 transition"
          >Contact Support</a
        >
      </section>
    </div>
      ...
      next: (res) => {
        ...
        if (res.success) {
          this.toastService.show({
            ...
            message: `+${res.credits_added} credits toegevoegd. Nieuw saldo: ${res.new_balance}.`,
            variant: 'success',
          });
          ...
          this.toastService.show({
            ...
            message: 'Je credits worden zo meteen bijgewerkt.',
            variant: 'info',
          });
        }
      },
